...
for i in range(len(value)):
    ...
    # Find all elements with class "colonne"
    colonne_elements = soup.find_all(class_="colonne")
    title_elements = soup.find_all("h1")
    cellule_elements = soup.find_all(class_="cellule")

    # Extract text from the selected elements
    text_colonne = "\n".join(colonne.get_text() for colonne in colonne_elements)
    text_title = "\n".join(title.get_text() for title in title_elements)
    text_cellule = "\n".join(cellule.get_text() for cellule in cellule_elements)

    # Remove empty lines from text_colonne
    text_colonne_lines = [line.strip() for line in text_colonne.splitlines() if line.strip()]
    text_cellule_lines = [line.strip() for line in text_cellule.splitlines() if line.strip()]

    # Join the non-empty lines back together for text_colonne
    text_colonne = "\n".join(text_colonne_lines)
    text_cellule = "\n".join(text_cellule_lines)

    # Create the combined text with h1 element and colonne element
    combined_text = text_title + "\n" + text_colonne + "\n" + text_cellule 

    # Create the filename
    filename = re.sub(r'[\/\-\.?]', '_', value[i]) + '.txt'
